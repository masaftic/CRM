...
{
    public static async Task<IResult> Handle(string dealId, TransitionDealRequest request, [FromServices] ISalesUnitOfWork uow)
    {
        ...
        try
        {
            deal.TransitionToStage(StageId.Create(request.StageId));
            await uow.SaveChangesAsync();
        }
        catch (InvalidOperationException ex)
        {
            return AppError.Validation("Deal.Transition.Invalid", ex.Message).ToProblemDetails();
        }
